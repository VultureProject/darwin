...
class Filter():
    ...
    def __init__(self, path=None, config_file=None, filter_name="filter", socket_path=None, monitoring_socket_path=None, pid_file=None, output="NONE", next_filter_socket_path="no", nb_thread=1, cache_size=0, thresold=101, log_level="-z", logger=None):
    ...
    def check_start(self):
        if not os.path.exists(self.pid):
            self.logger.error("No PID file at start, maybe filter crashed")
            return False
        return True

    def check_stop(self):
        if os.path.exists(self.pid):
            self.logger.error("PID file not removed when stopping, maybe filter crashed")
            return False
        return True
    ...
    def stop(self):
        ...
        try:
            ...
            self.logger.error("Unable to stop filter properly... Killing it.")
            self.process.kill()
            self.process.wait()
            return False
    ...
    def valgrind_stop(self):
        ...
        if self.process.returncode == self.error_code :
            ...
            self.logger.error("Valgrind returned error code: {}".format(self.process.returncode))
            self.logger.error("Valgrind error: {}".format(err))
            return False
